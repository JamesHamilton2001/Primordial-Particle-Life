...
def visualise_intercounts(t_interdists, interdist_stats):
    ...
    ax.set_xlabel("Type 2", labelpad=15, color="w")
    ax.set_ylabel("Type 1", labelpad=15, color="w")
    ax.set_zlabel("Interactions", labelpad=15, color="w")
    ...
    ax.set_zticklabels([f"{t}" for t in zticks], color="w", fontsize=8)
    
    for pos in range(T):
        ax.text(pos+0.5, -0.5, -0.25*zt, f"T{pos}", "x", color="w", fontsize=8, ha="center", va="center")
        ax.text(-0.5, pos+0.5, -0.25*zt, f"T{pos}", "y", color="w", fontsize=8, ha="center", va="center")

    for xt, t_data in zip(xyticks, typed_intercounts):
        for yt, h in zip(xyticks, t_data):
            xpos, ypos = xt+0.5, yt+0.5
            ax.text(xpos, ypos, h+0.1*zt, f"{h}", color="w", fontsize=6, ha="center", va="center", zorder=9999)
