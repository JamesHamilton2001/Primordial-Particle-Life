...
void SpatialHash::map()
{
    ...
    // taurean edge mapping
    for (unsigned int r = 0; r < 4; r++) {
        for (unsigned int c = 1; c <= S; c++) {
            for (Particle* ptr : taureanGrid[(this->*innerEdgeIdx[r][0])(c)]
                                            [(this->*innerEdgeIdx[r][1])(c)]) {
                Particle p = *ptr;
            ...
            for (Particle& p : edgeWrapCells[r][c-1])
                taureanGrid[(this->*outerEdgeIdx[r][0])(c)]
                           [(this->*outerEdgeIdx[r][1])(c)].emplace_back(&p);
    }
}
